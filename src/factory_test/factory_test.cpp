...
void FactoryTest::init()
{
    ...
    // NOTE: do NOT call tfLuna.warmUp() here (or anywhere in this early boot
    // path). It was tried once — right after _power_on()'s latch — and
    // caused boot failures on a weak/low battery: it drives I2C + the
    // sensor's laser for up to 200ms during the most voltage-fragile instant
    // of boot, which can sag the rail past the ESP32-S3 brownout threshold.
    // AutoShoot uses a plain tfLuna.begin() on mode entry instead
    // (auto_shoot.cpp) — by then boot + display + menu already proved the
    // supply can hold.
